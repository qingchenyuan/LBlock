...
void encrypt_decrypt(uint64_t IV,uint16_t a,uint64_t b,uint64_t c,uint64_t d,uint32_t keymod,uint32_t rounds,u32 mod);
...
int main()
{
	...
	u32 mod;
	uint32_t level;
	xil_printf("choose security level(1,2,3,4):");
	scanf("%ld",&level);
	if(level == 1)
	{
		keymod = 0;
	...
	{
		xil_printf("enter an invalid security level!!!");
		exit(0);
	}
	...
	if(mod != 0 && mod !=1)
	{
		xil_printf("enter an invalid mod!!!");
		exit(0);
	}
	init_LBlock();
	file_mangement(mod);
	read_from_SD(mod);
	encrypt_decrypt(IV,a,b,c,d,keymod,rounds,mod);
	f_close(&file1);
	f_close(&file2);
	cleanup_platform();
	return 0;
		...
		if(mod == 1)
		{
			...
			if(((ind -1) != 0 && (ind-1) % 1023 == 0) || mem_buff[ind]==0)
			{
				if(XCodec_IsIdle(&Codec))
				{
					...
					XCodec_Set_c(&Codec, c);
					XCodec_Set_d(&Codec, d);
					XCodec_Set_IV(&Codec, IV);
					XCodec_Set_keymod(&Codec, keymod);
