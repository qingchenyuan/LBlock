...
void codec(uint64_t IV,uint64_t text[1024], uint16_t a, uint64_t b,uint64_t c,uint64_t d,uint32_t keymod,uint32_t rounds, bool mod, uint64_t ans[1024])
{
	...
	#pragma HLS interface s_axilite port=IV bundle=s_axi
	...
	{
		if(text[j] !=0 )//1024g counter
		{
			left = (I[j]>>32) & 0xFFFFFFFF;
			right = I[j] & 0xFFFFFFFF;
			// generate round key according keymod
			if(keymod == 0)
			{
				roundkey_less(a,b,rounds,rk1);
			}
			if(keymod == 1)
			{
