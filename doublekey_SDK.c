...
void encrypt_decrypt(uint16_t a,uint64_t b,uint64_t c,uint64_t d,uint32_t keymod,u32 mod);
...
int main()
{
	...
	uint32_t keymod;
	u32 mod;
	xil_printf("choose keymod(0-80 bit key,1-1288 bit key):");
	scanf("%ld",&keymod);
	...
	file_mangement(mod);
	read_from_SD(mod);
	encrypt_decrypt(a,b,c,d,keymod,mod);
	f_close(&file1);
	f_close(&file2);
	cleanup_platform();
	return 0;
